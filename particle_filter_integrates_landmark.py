    ...
    def creating_particles(self, Ut, Zt): # form of Ut and Zt gotta be different format. This is just for referace.
        ...
        #     ----------------------landmark, Ct-------------------------------
            if self.Ct:# Ct never seen before: Ct = Matrix that discribe how to map the state Xt to an observation Zt
                ...
                landmark.landmark(Ct)
                l_pos = np.linalg.inv(Z_hat);
                ...
                np.append(landmark.landmark_sigs, sig) #append sig list with new sig
                np.append(landmark.landmark_locations, l_pos) #append landmark list with new landmark location (x,y)
                np.append(landmark.landmark_Qt, Qt) #append landmark list with new landmark uncertainty Qt
                ...
                prev_l_pos, prev_sig, Qt = landmark.landmark(Ct)
                ...
                K = calc_kalmangain(prev_sig, H, particle, Q)   
                ...
                landmark.update_landmark(Ct, (np.identity(2) - K @ H) @ prev_sig, prev_l_pos + K @ (Zt - Z_hat), update_Qt(Qt))
...
if __name__ == '__main__':
    ...
    particle_set = particle.creating_particles(1, math.pi/3)
